(function() {
    ...
    function populateNetworkSelect() {
        var default_selection = "BTC - Bitcoin";
        if ("coin" in location.queryString) {
            var sel2 = location.queryString["coin"];
            for (var i=0; i<networks.length; i++) {
                if (networks[i].name.startsWith(sel2)) {
                    default_selection = networks[i].name;
                    break;
                }
            }
        }
        for (var i=0; i<networks.length; i++) {
            ...
            if (network.name == default_selection) {
